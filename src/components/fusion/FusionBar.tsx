    ...
    <div className="bg-gray-800 border border-gray-700 rounded px-3 py-2 flex flex-col md:flex-row md:items-center md:justify-between text-white text-sm gap-2">
      {fusion.bought ? (
        <div
          ...
          style={{ backgroundColor: fusion.output.color }}
        >
          <span className="text-xl">
            {ResourceIcons[fusion.output.key as ResourceKey]}
          </span>
          <span className="font-medium">{fusion.output.key}</span>
        </div>
      ) : (
      ...
      >
        {fusion.bought ? "Unlocked" : "Unknown"}
      </span>
    </div>
  );
}
